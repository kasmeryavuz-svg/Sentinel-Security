...
class ProductionBytecodePolicyVerifierTest {
    ...
    @Test
    fun `wipeDevice non-zero constant is rejected`() {
        val violations = verify(
            ":device-management-impl",
            compileJava(
                authorizedFactoryResetResultStub(),
                factoryResetWipeJava("manager.wipeDevice(1);"),
            ),
        )
        assertTrue(violations.any { "exact integer constant 0" in it && "wipeDevice" in it })
        assertTrue(violations.none { "Checkpoint 17B-blocked" in it && "wipeDevice" in it })
    }

    @Test
    fun `wipeDevice local-variable flag is rejected`() {
        val violations = verify(
            ":device-management-impl",
            compileJava(
                authorizedFactoryResetResultStub(),
                factoryResetWipeJava("int flags = 0; manager.wipeDevice(flags);"),
            ),
        )
        assertTrue(violations.any { "exact integer constant 0" in it && "wipeDevice" in it })
    }
    ...
    @Test
    fun `wipeDevice field flag is rejected`() {
        val classes = writeClassFiles(
            factoryResetWipeDeviceBytecode {
                visitFieldInsn(
                    Opcodes.GETSTATIC,
    ...
    @Test
    fun `wipeDevice calculated flag is rejected`() {
        val classes = writeClassFiles(
            factoryResetWipeDeviceBytecode {
    ...
    @Test
    fun `wipeDevice exact ICONST_0 bytecode is accepted from the factory-reset origin`() {
        val classes = writeClassFiles(
            factoryResetWipeDeviceBytecode {
                visitInsn(Opcodes.ICONST_0)
            },
    ...
    @Test
    fun `multiple wipeDevice calls are rejected even when one is constant zero`() {
        val classes = writeClassFiles(
            factoryResetWipeDeviceBytecode(extraWipeDeviceZero = true) {
                visitInsn(Opcodes.ICONST_0)
            },
        )
        val violations = verify(":device-management-impl", classes)
        assertTrue(violations.any { "multiple DevicePolicyManager.wipeDevice(I)V" in it })
        assertTrue(violations.any { "exactly one" in it && "wipeDevice" in it })
    }

    @Test
    fun `extra wipeDevice is rejected even when factory-reset uses constant zero`() {
        val classes = compileJava(
            authorizedFactoryResetResultStub(),
            factoryResetWipeJava("manager.wipeDevice(0);"),
            "attack/SecondWipe.java" to
                """
                package attack;
                import android.app.admin.DevicePolicyManager;
                public final class SecondWipe {
                    void attack(DevicePolicyManager manager) {
                        manager.wipeDevice(0);
                    }
                }
                """.trimIndent(),
    ...
    @Test
    fun `wipeDevice method handle is rejected`() {
        val classes = writeClassFiles(factoryResetWipeDeviceHandleBytecode())
        val violations = verify(":device-management-impl", classes)
        assertTrue(violations.any { "method handle" in it && "wipeDevice" in it })
        assertTrue(violations.any { "exact integer constant 0" in it })
    }
    ...
    fun `performAuthorizedFactoryReset is origin-bound to the future executor`() {
        ...
        assertTrue(
            violations.any {
                "AndroidFutureDestructiveExecutor.onAuthorizedHandoff" in it &&
                    "FutureDestructiveHandoffAcknowledgement" in it
            },
        )
    }
            ...
            *factoryResetPortStubs(),
            "com/example/devicemanagement/destructive/AndroidFutureDestructiveExecutor.java" to
                ...
                public final class AndroidFutureDestructiveExecutor {
                    ...
                    protected FutureDestructiveHandoffAcknowledgement onAuthorizedHandoff() {
                        factoryReset.performAuthorizedFactoryReset();
                        return null;
                    }
                }
                """.trimIndent(),
    ...
    @Test
    fun `overloaded onAuthorizedHandoff cannot invoke the factory-reset port`() {
        val classes = compileJava(
            *factoryResetPortStubs(),
            "com/example/devicemanagement/destructive/AndroidFutureDestructiveExecutor.java" to
                """
        ...
        val violations = verify(":sensitive-actions", classes)
        assertTrue(violations.any { "authorized factory-reset" in it })
        assertTrue(violations.any { "onAuthorizedHandoff(Ljava/lang/String;)" in it })
    }

    @Test
    fun `wrong onAuthorizedHandoff descriptor cannot invoke the factory-reset port`() {
        val classes = compileJava(
            *factoryResetPortStubs(),
            "com/example/devicemanagement/destructive/AndroidFutureDestructiveExecutor.java" to
                """
                ...
                public final class AndroidFutureDestructiveExecutor {
                    ...
                    public Object onAuthorizedHandoff() {
        ...
        val violations = verify(":sensitive-actions", classes)
        assertTrue(violations.any { "authorized factory-reset" in it })
        assertTrue(violations.any { "onAuthorizedHandoff()Ljava/lang/Object;" in it })
    }

    @Test
    fun `wrong performAuthorizedFactoryReset descriptor is rejected from the exact executor method`() {
        val classes = compileJava(
            "com/example/devicemanagement/destructive/AuthorizedFactoryResetResult.java" to
                """
                package com.example.devicemanagement.destructive;
            ...
            "com/example/devicemanagement/destructive/AuthorizedFactoryResetPort.java" to
                ...
                public interface AuthorizedFactoryResetPort {
                    ...
                    AuthorizedFactoryResetResult performAuthorizedFactoryReset(String extra);
                }
                """.trimIndent(),
            "com/example/devicemanagement/destructive/FutureDestructiveHandoffAcknowledgement.java" to
                """
                package com.example.devicemanagement.destructive;
                public class FutureDestructiveHandoffAcknowledgement {}
                """.trimIndent(),
            "com/example/devicemanagement/destructive/AndroidFutureDestructiveExecutor.java" to
                """
                package com.example.devicemanagement.destructive;
                public final class AndroidFutureDestructiveExecutor {
                    ...
                    protected FutureDestructiveHandoffAcknowledgement onAuthorizedHandoff() {
                        factoryReset.performAuthorizedFactoryReset("extra");
                        return null;
                    }
                }
                """.trimIndent(),
        ...
        val violations = verify(":sensitive-actions", classes)
        assertTrue(violations.any { "authorized factory-reset" in it })
        assertTrue(violations.any { "performAuthorizedFactoryReset(Ljava/lang/String;)" in it })
    }

    @Test
    fun `factory-reset port method handle is rejected even from the exact executor method`() {
        val classes = compileJava(
            *factoryResetPortStubs(),
            "com/example/devicemanagement/destructive/AndroidFutureDestructiveExecutor.java" to
                """
                    ...
                    protected FutureDestructiveHandoffAcknowledgement onAuthorizedHandoff() {
                        try {
                            MethodHandles.lookup().findVirtual(
                                AuthorizedFactoryResetPort.class,
                                "performAuthorizedFactoryReset",
                                MethodType.methodType(AuthorizedFactoryResetResult.class)
                            );
        ...
        val violations = verify(":sensitive-actions", classes)
        assertTrue(
            violations.any { "method handles" in it || "authorized factory-reset" in it },
        )
    }

    @Test
    fun `factory-reset port method reference is rejected`() {
        val classes = compileJava(
            *factoryResetPortStubs(),
            "com/example/devicemanagement/destructive/AndroidFutureDestructiveExecutor.java" to
                """
                ...
                public final class AndroidFutureDestructiveExecutor {
                    ...
                    protected FutureDestructiveHandoffAcknowledgement onAuthorizedHandoff() {
                        Supplier<AuthorizedFactoryResetResult> ignored =
                            factoryReset::performAuthorizedFactoryReset;
                        return null;
                    }
                }
        ...
        assertTrue(
            violations.any {
                "invokedynamic" in it || "method handles" in it || "authorized factory-reset" in it
            },
        )
    }

    @Test
    fun `alternate factory-reset port owner is rejected`() {
        val classes = compileJava(
            *factoryResetPortStubs(),
            "attack/AlternateFactoryResetPort.java" to
                """
                package attack;
                import com.example.devicemanagement.destructive.AuthorizedFactoryResetResult;
                public final class AlternateFactoryResetPort {
                    public AuthorizedFactoryResetResult performAuthorizedFactoryReset() {
                        return null;
                    }
                }
            ...
            "com/example/devicemanagement/destructive/AndroidFutureDestructiveExecutor.java" to
                """
                package com.example.devicemanagement.destructive;
                import attack.AlternateFactoryResetPort;
                public final class AndroidFutureDestructiveExecutor {
                    private final AlternateFactoryResetPort factoryReset;
                    AndroidFutureDestructiveExecutor(AlternateFactoryResetPort factoryReset) {
                        this.factoryReset = factoryReset;
                    }
                    protected FutureDestructiveHandoffAcknowledgement onAuthorizedHandoff() {
                        factoryReset.performAuthorizedFactoryReset();
        ...
        val violations = verify(":sensitive-actions", classes)
        assertTrue(violations.any { "authorized factory-reset" in it })
        assertTrue(violations.any { "AlternateFactoryResetPort" in it })
    }

    @Test
    fun `retainForProduction is origin-bound to device-management composition`() {
        val classes = compileJava(
            *retainForProductionStubs(),
            "attack/RogueDestructiveRetain.java" to
                ...
                public final class RogueDestructiveRetain {
                    void attack(ProductionDestructiveRealChain chain) {
                        chain.retainForProduction(null, null, null, null);
                    }
                }
                """.trimIndent(),
        )
        ...
        assertTrue(violations.any { "retainProductionDestructiveImplementation" in it })
    }
    ...
    fun `authorized composition may retain the production destructive chain`() {
        val classes = compileJava(
            *retainForProductionStubs(),
            authorizedRetainCompositionJava(),
        )
    ...
    @Test
    fun `retainForProduction rejects wrong caller descriptor and method handles`() {
            ...
            *retainForProductionStubs(),
            "com/example/devicemanagement/management/DeviceManagementComposition.java" to
                ...
                public final class DeviceManagementComposition {
                    ProductionDestructiveRetainer create(
                        ProductionDestructiveRealChain chain
                    ) {
                        return chain.retainForProduction(null, null, null, null);
                    }
                }
                """.trimIndent(),
        )
        val wrongCallerViolations = verify(":device-management-impl", wrongCaller)
        assertTrue(wrongCallerViolations.any { "production destructive retainer" in it })
        assertTrue(wrongCallerViolations.any { ".create(" in it })
        ...
        val handle = compileJava(
            *retainForProductionStubs(),
            "com/example/devicemanagement/management/DeviceManagementComposition.java" to
                ...
                public final class DeviceManagementComposition {
                    private ProductionDestructiveRetainer retainProductionDestructiveImplementation(
                        Context context,
                        StructuredLogger logger,
                        DeviceManagementLogger deviceManagementLogger,
                        AndroidDevicePolicyPlatform platform
                    ) {
                        try {
                            MethodHandles.lookup().findVirtual(
                                ProductionDestructiveRealChain.class,
                                "retainForProduction",
                                MethodType.methodType(
                                    ProductionDestructiveRetainer.class,
                                    AuthorizedFactoryResetPort.class,
                                    DestructiveLiveFactsSource.class,
                                    MonotonicTimeSource.class,
                                    RuntimeDestructiveSafetyDurability.class
                                )
                            );
                        } catch (Exception ignored) {}
                        return null;
        ...
        val handleViolations = verify(":device-management-impl", handle)
        assertTrue(
            handleViolations.any { "method handles" in it || "production destructive retainer" in it },
        )
    }

    @Test
    fun `retainForProduction rejects alternate owner even from the exact composition method`() {
        val classes = compileJava(
            *retainForProductionStubs(),
            "attack/AlternateDestructiveChain.java" to
                """
                package attack;
                import com.example.devicemanagement.destructive.AuthorizedFactoryResetPort;
                import com.example.devicemanagement.destructive.DestructiveLiveFactsSource;
                ...
                import com.example.devicemanagement.destructive.RuntimeDestructiveSafetyDurability;
                import com.example.devicemanagement.integration.MonotonicTimeSource;
                public final class AlternateDestructiveChain {
                    public ProductionDestructiveRetainer retainForProduction(
                        AuthorizedFactoryResetPort factoryReset,
                        DestructiveLiveFactsSource liveFacts,
                        MonotonicTimeSource clock,
                        RuntimeDestructiveSafetyDurability durability
                    ) {
                        return null;
                    }
                }
            ...
            "com/example/devicemanagement/management/DeviceManagementComposition.java" to
                ...
                package com.example.devicemanagement.management;
                import android.content.Context;
                import attack.AlternateDestructiveChain;
                import com.example.devicemanagement.destructive.ProductionDestructiveRetainer;
                import com.example.devicemanagement.logging.StructuredLogger;
                public final class DeviceManagementComposition {
                    private ProductionDestructiveRetainer retainProductionDestructiveImplementation(
                        ...
                        return new AlternateDestructiveChain().retainForProduction(
                            null, null, null, null
                        );
                    }
                }
        ...
        val violations = verify(":device-management-impl", classes)
        assertTrue(violations.any { "production destructive retainer" in it })
        assertTrue(violations.any { "AlternateDestructiveChain" in it })
        assertTrue(violations.any { "retainProductionDestructiveImplementation" in it })
    }
    ...
    @Test
    fun `recovery class cannot reference Checkpoint 19A decision`() {
        val classes = compileJava(
            "com/example/devicemanagement/destructive/Checkpoint19ADecision.java" to
                """
                package com.example.devicemanagement.destructive;
                public final class Checkpoint19ADecision {}
                """.trimIndent(),
            "com/example/devicemanagement/recovery/RogueRecoveryCheckpoint19A.java" to
                """
                package com.example.devicemanagement.recovery;
                import com.example.devicemanagement.destructive.Checkpoint19ADecision;
                public final class RogueRecoveryCheckpoint19A {
                    Checkpoint19ADecision decision;
                }
                """.trimIndent(),
        )
    ...
    @Test
    fun `recovery class cannot reference Checkpoint 19C decision`() {
        val classes = compileJava(
            "com/example/devicemanagement/destructive/Checkpoint19CDecision.java" to
                """
                package com.example.devicemanagement.destructive;
                public final class Checkpoint19CDecision {}
                """.trimIndent(),
            "com/example/devicemanagement/recovery/RogueRecoveryCheckpoint19C.java" to
                """
                package com.example.devicemanagement.recovery;
                import com.example.devicemanagement.destructive.Checkpoint19CDecision;
                public final class RogueRecoveryCheckpoint19C {
                    Checkpoint19CDecision decision;
                }
                """.trimIndent(),
        )
        ...
        val violations = verify(":sensitive-actions", classes)
        assertTrue(violations.any { "recovery code" in it })
        assertTrue(violations.any { "Checkpoint19CDecision" in it })
    }
    ...
    @Test
    fun `recovery class cannot reference Checkpoint 19E decision`() {
        val classes = compileJava(
            "com/example/devicemanagement/destructive/Checkpoint19EDecision.java" to
                """
                package com.example.devicemanagement.destructive;
                public final class Checkpoint19EDecision {}
                """.trimIndent(),
            "com/example/devicemanagement/recovery/RogueRecoveryCheckpoint19E.java" to
                """
                package com.example.devicemanagement.recovery;
                import com.example.devicemanagement.destructive.Checkpoint19EDecision;
                public final class RogueRecoveryCheckpoint19E {
                    Checkpoint19EDecision decision;
                }
                """.trimIndent(),
        )
        ...
        val violations = verify(":sensitive-actions", classes)
        assertTrue(violations.any { "recovery code" in it })
        assertTrue(violations.any { "Checkpoint19EDecision" in it })
    }
    ...
    @Test
    fun `recovery class cannot reference Checkpoint 19G decision`() {
        val classes = compileJava(
            "com/example/devicemanagement/destructive/Checkpoint19GDecision.java" to
                """
                package com.example.devicemanagement.destructive;
                public final class Checkpoint19GDecision {}
                """.trimIndent(),
            "com/example/devicemanagement/recovery/RogueRecoveryCheckpoint19G.java" to
                """
                package com.example.devicemanagement.recovery;
                import com.example.devicemanagement.destructive.Checkpoint19GDecision;
                public final class RogueRecoveryCheckpoint19G {
                    Checkpoint19GDecision decision;
                }
                """.trimIndent(),
        )
        ...
        val violations = verify(":sensitive-actions", classes)
        assertTrue(violations.any { "recovery code" in it })
        assertTrue(violations.any { "Checkpoint19GDecision" in it })
    }
    ...
    @Test
    fun `recovery class cannot reference Checkpoint 19P governance observation`() {
        val classes = compileJava(
            "com/example/devicemanagement/destructive/Checkpoint19PGovernanceObservation.java" to
                """
                package com.example.devicemanagement.destructive;
                public final class Checkpoint19PGovernanceObservation {}
                """.trimIndent(),
            "com/example/devicemanagement/recovery/RogueRecoveryCheckpoint19P.java" to
                """
                package com.example.devicemanagement.recovery;
                import com.example.devicemanagement.destructive.Checkpoint19PGovernanceObservation;
                public final class RogueRecoveryCheckpoint19P {
                    Checkpoint19PGovernanceObservation observation;
                }
                """.trimIndent(),
        )
        ...
        val violations = verify(":sensitive-actions", classes)
        assertTrue(violations.any { "recovery code" in it })
        assertTrue(violations.any { "Checkpoint19PGovernanceObservation" in it })
    }
    ...
    fun `recovery class cannot reference Checkpoint 19H decision`() {
        val classes = compileJava(
            "com/example/devicemanagement/destructive/Checkpoint19HDecision.java" to
                """
                package com.example.devicemanagement.destructive;
                public final class Checkpoint19HDecision {}
                """.trimIndent(),
            "com/example/devicemanagement/recovery/RogueRecoveryCheckpoint19H.java" to
                """
                package com.example.devicemanagement.recovery;
                import com.example.devicemanagement.destructive.Checkpoint19HDecision;
                public final class RogueRecoveryCheckpoint19H {
                    Checkpoint19HDecision decision;
                }
                """.trimIndent(),
        )
        ...
        val violations = verify(":sensitive-actions", classes)
        assertTrue(violations.any { "recovery code" in it })
        assertTrue(violations.any { "Checkpoint19HDecision" in it })
    }
            ...
            "attack/RogueProductionConfirmation.java" to
                ...
                public final class RogueProductionConfirmation {
                    void forge(ProductionDestructiveHumanConfirmationSource source) {
                        source.confirm(null, null, null, null, null, null, 0L);
                    }
                }
                """.trimIndent(),
        )
    ...
    @Test
    fun `production confirmation source rejects overload alternate owner method handles method references and reflection`() {
        val overload = compileJava(
            *productionConfirmationStubs(),
            "com/example/devicemanagement/destructive/ProductionDestructiveRealChainOrchestrator.java" to
                """
                package com.example.devicemanagement.destructive;
                public final class ProductionDestructiveRealChainOrchestrator {
                    private ProductionDestructiveHumanConfirmationSource confirmationSource;
                    public FutureDestructiveHandoffResult assembleAlreadyBoundDeviceFactoryReset(
                        String extra
                    ) {
                        confirmationSource.confirm(null, null, null, null, null, null, 0L);
                        return null;
                    }
                }
                """.trimIndent(),
        ...
        val overloadViolations = verify(":sensitive-actions", overload)
        assertTrue(overloadViolations.any { "production confirmation source" in it })

        val alternate = compileJava(
            *productionConfirmationStubs(),
            "attack/AlternateConfirmationSource.java" to
                """
                package attack;
                import com.example.devicemanagement.destructive.DestructiveAttemptLease;
                import com.example.devicemanagement.destructive.DestructiveArtifactIdentity;
                import com.example.devicemanagement.destructive.DestructiveCorrelationId;
                import com.example.devicemanagement.destructive.DestructiveHumanConfirmationResult;
                import com.example.devicemanagement.destructive.DestructiveOperatorChallenge;
                import com.example.devicemanagement.destructive.DestructiveScope;
                import com.example.devicemanagement.destructive.DestructiveTargetBinding;
                public final class AlternateConfirmationSource {
                    public DestructiveHumanConfirmationResult confirm(
            ...
            "com/example/devicemanagement/destructive/ProductionDestructiveRealChainOrchestrator.java" to
                """
                package com.example.devicemanagement.destructive;
                import attack.AlternateConfirmationSource;
                public final class ProductionDestructiveRealChainOrchestrator {
                    public Object assembleAlreadyBoundDeviceFactoryReset(
                        ProductionBoundDeviceFactoryResetAttempt attempt
                    ) {
                        return new AlternateConfirmationSource().confirm(
        ...
        val alternateViolations = verify(":sensitive-actions", alternate)
        assertTrue(alternateViolations.any { "production confirmation source" in it || "human confirmation" in it })
            ...
            "com/example/devicemanagement/destructive/ProductionDestructiveRealChainOrchestrator.java" to
                ...
                public final class ProductionDestructiveRealChainOrchestrator {
                    public Object assembleAlreadyBoundDeviceFactoryReset(
                        ...
                        try {
                            MethodHandles.lookup().findVirtual(
                                ProductionDestructiveHumanConfirmationSource.class,
                                "confirm",
                                MethodType.methodType(
                                    DestructiveHumanConfirmationResult.class,
                                    DestructiveCorrelationId.class,
                                    DestructiveTargetBinding.class,
                                    DestructiveScope.class,
                                    DestructiveArtifactIdentity.class,
                                    DestructiveOperatorChallenge.class,
                                    DestructiveAttemptLease.class,
                                    long.class
                                )
                            );
        ...
        val handleViolations = verify(":sensitive-actions", handle)
        assertTrue(
            handleViolations.any {
                "production confirmation source" in it || "method handle" in it || "method handles" in it
            },
        )

        val methodReference = compileKotlin(
            *productionConfirmationKotlinStubs(),
            "attack/RogueConfirmationHandle.kt" to
                """
                package attack
                import com.example.devicemanagement.destructive.ProductionDestructiveHumanConfirmationSource
                class RogueConfirmationHandle {
                    fun forge(
                        source: ProductionDestructiveHumanConfirmationSource,
                    ) = source::confirm
                }
                """.trimIndent(),
        )
        val methodReferenceViolations = verify(":sensitive-actions", methodReference)
        assertTrue(
            methodReferenceViolations.any {
                "production confirmation source" in it ||
                    "invokedynamic" in it ||
                    "method handle" in it ||
                    "method handles" in it
            },
        )

        val reflection = compileJava(
            *productionConfirmationStubs(),
            "attack/RogueConfirmationReflection.java" to
                """
                package attack;
                import com.example.devicemanagement.destructive.ProductionDestructiveHumanConfirmationSource;
                public final class RogueConfirmationReflection {
                    Object forge(ProductionDestructiveHumanConfirmationSource source) throws Exception {
                        return source.getClass().getDeclaredMethod("confirm").invoke(source);
                    }
                }
                """.trimIndent(),
        )
        val reflectionViolations = verify(":sensitive-actions", reflection)
        assertTrue(
            reflectionViolations.any { "reflective lookup" in it || "Java reflection" in it },
        )
    }

    @Test
    fun `retainForProduction may construct the orchestrator confirmation source and null trusted sources`() {
        val classes = compileJava(
            *productionInternalInitStubs(),
            authorizedRetainConstructionJava(),
        )
        val violations = verify(":sensitive-actions", classes)
        assertTrue(
            violations.none { "constructs production real-chain type" in it },
            violations.joinToString("\n"),
        )
    }

    @Test
    fun `rogue caller cannot construct the production orchestrator or confirmation source`() {
        val classes = compileJava(
            *productionInternalInitStubs(),
            "attack/RogueOrchestratorInit.java" to
                """
                package attack;
                import com.example.devicemanagement.destructive.ProductionDestructiveHumanConfirmationSource;
                import com.example.devicemanagement.destructive.ProductionDestructiveRealChainOrchestrator;
                import com.example.devicemanagement.destructive.ProductionDestructiveUtcClock;
                public final class RogueOrchestratorInit {
                    void forge() {
                        new ProductionDestructiveUtcClock();
                        new ProductionDestructiveHumanConfirmationSource(
                            null, null, null, null, null
                        );
                        new ProductionDestructiveRealChainOrchestrator(
        ...
        val violations = verify(":sensitive-actions", classes)
        assertTrue(violations.any { "constructs production real-chain type" in it })
        assertTrue(violations.any { "ProductionDestructiveRealChainOrchestrator" in it })
        assertTrue(violations.any { "ProductionDestructiveHumanConfirmationSource" in it })
    }

    @Test
    fun `recovery class cannot reference 19D confirmation seams`() {
        val classes = compileJava(
            "com/example/devicemanagement/destructive/ProductionDestructiveUtcClock.java" to
                """
                package com.example.devicemanagement.destructive;
                public final class ProductionDestructiveUtcClock {}
                """.trimIndent(),
            "com/example/devicemanagement/destructive/DestructiveTrustedArtifactExpectationSource.java" to
                """
                package com.example.devicemanagement.destructive;
                public interface DestructiveTrustedArtifactExpectationSource {}
                """.trimIndent(),
            "com/example/devicemanagement/recovery/RogueRecovery19DSeams.java" to
                """
                package com.example.devicemanagement.recovery;
                import com.example.devicemanagement.destructive.DestructiveTrustedArtifactExpectationSource;
                import com.example.devicemanagement.destructive.ProductionDestructiveUtcClock;
                public final class RogueRecovery19DSeams {
                    DestructiveTrustedArtifactExpectationSource source;
                    ProductionDestructiveUtcClock clock;
                }
                """.trimIndent(),
        )
        val violations = verify(":sensitive-actions", classes)
        assertTrue(violations.any { "recovery code" in it })
        assertTrue(violations.any { "ProductionDestructiveUtcClock" in it })
        assertTrue(violations.any { "DestructiveTrustedArtifactExpectationSource" in it })
    }
    ...
    private fun factoryResetPortStubs(): Array<Pair<String, String>> {
        return arrayOf(
            "com/example/devicemanagement/destructive/AuthorizedFactoryResetResult.java" to
                """
                package com.example.devicemanagement.destructive;
    ...
    private fun retainForProductionStubs(): Array<Pair<String, String>> {
        return arrayOf(
            "com/example/devicemanagement/destructive/AuthorizedFactoryResetPort.java" to
                """
                package com.example.devicemanagement.destructive;
                public interface AuthorizedFactoryResetPort {}
                """.trimIndent(),
            "com/example/devicemanagement/destructive/DestructiveLiveFactsSource.java" to
                """
                package com.example.devicemanagement.destructive;
                public interface DestructiveLiveFactsSource {}
                """.trimIndent(),
            "com/example/devicemanagement/integration/MonotonicTimeSource.java" to
                """
                package com.example.devicemanagement.integration;
                public interface MonotonicTimeSource {}
                """.trimIndent(),
            "com/example/devicemanagement/destructive/RuntimeDestructiveSafetyDurability.java" to
                """
                package com.example.devicemanagement.destructive;
                public class RuntimeDestructiveSafetyDurability {}
                """.trimIndent(),
            "com/example/devicemanagement/destructive/ProductionDestructiveRetainer.java" to
                """
                package com.example.devicemanagement.destructive;
                public class ProductionDestructiveRetainer {}
                """.trimIndent(),
            "com/example/devicemanagement/logging/StructuredLogger.java" to
                """
                package com.example.devicemanagement.logging;
                public interface StructuredLogger {}
                """.trimIndent(),
            "com/example/devicemanagement/management/DeviceManagementLogger.java" to
                """
                package com.example.devicemanagement.management;
                public interface DeviceManagementLogger {}
                """.trimIndent(),
            "com/example/devicemanagement/management/AndroidDevicePolicyPlatform.java" to
                """
                package com.example.devicemanagement.management;
                public class AndroidDevicePolicyPlatform {}
                """.trimIndent(),
            "com/example/devicemanagement/destructive/ProductionDestructiveRealChain.java" to
                ...
                public final class ProductionDestructiveRealChain {
                    public static final ProductionDestructiveRealChain INSTANCE =
                        new ProductionDestructiveRealChain();
                    public ProductionDestructiveRetainer retainForProduction(
                        AuthorizedFactoryResetPort factoryReset,
                        DestructiveLiveFactsSource liveFacts,
                        MonotonicTimeSource clock,
    ...
    private fun authorizedRetainCompositionJava(): Pair<String, String> {
        return "com/example/devicemanagement/management/DeviceManagementComposition.java" to
            """
            package com.example.devicemanagement.management;
            import android.content.Context;
            import com.example.devicemanagement.destructive.ProductionDestructiveRealChain;
    ...
    private fun productionConfirmationStubs(): Array<Pair<String, String>> {
        return arrayOf(
            ...
            "com/example/devicemanagement/destructive/DestructiveHumanConfirmationAuthority.java" to
                ...
                """.trimIndent(),
            "com/example/devicemanagement/destructive/DestructiveTrustedPerAttemptConfirmationRecordSource.java" to
                """
                package com.example.devicemanagement.destructive;
                public interface DestructiveTrustedPerAttemptConfirmationRecordSource {}
                """.trimIndent(),
            "com/example/devicemanagement/destructive/DestructiveUtcClock.java" to
                """
                package com.example.devicemanagement.destructive;
                public interface DestructiveUtcClock {}
                """.trimIndent(),
            "com/example/devicemanagement/destructive/DestructiveTrustedApprovedBuildRevisionSource.java" to
                """
                package com.example.devicemanagement.destructive;
                public interface DestructiveTrustedApprovedBuildRevisionSource {}
                """.trimIndent(),
            "com/example/devicemanagement/destructive/DestructiveTrustedArtifactExpectationSource.java" to
                """
                package com.example.devicemanagement.destructive;
            ...
            "com/example/devicemanagement/destructive/ProductionDestructiveHumanConfirmationSource.java" to
                ...
                public final class ProductionDestructiveHumanConfirmationSource {
                    public ProductionDestructiveHumanConfirmationSource(
                        DestructiveTrustedPerAttemptConfirmationRecordSource recordSource,
                        DestructiveUtcClock utcClock,
                        DestructiveTrustedApprovedBuildRevisionSource approvedBuildRevision,
                        DestructiveLiveFactsSource liveFacts,
                        DestructiveTrustedArtifactExpectationSource artifactExpectationSource
                    ) {}
                    public DestructiveHumanConfirmationResult confirm(
                        DestructiveCorrelationId correlationId,
                        DestructiveTargetBinding binding,
                        ...
                        return new DestructiveHumanConfirmationAuthority().confirm(
                            challenge, correlationId, binding, scope, artifactIdentity,
                            attemptLease, nowMonotonicMillis
                        );
                    }
                }
                """.trimIndent(),
    ...
    private fun authorizedOrchestratorJava(
        ...
        val confirmationField = if (withConfirmation) {
            "private ProductionDestructiveHumanConfirmationSource confirmationSource;"
        } else {
            ""
        }
    ...
    private fun productionConfirmationKotlinStubs(): Array<Pair<String, String>> {
        return arrayOf(
            "com/example/devicemanagement/destructive/DestructiveCorrelationId.kt" to
                """
                package com.example.devicemanagement.destructive
                class DestructiveCorrelationId
                """.trimIndent(),
            "com/example/devicemanagement/destructive/DestructiveTargetBinding.kt" to
                """
                package com.example.devicemanagement.destructive
                class DestructiveTargetBinding
                """.trimIndent(),
            "com/example/devicemanagement/destructive/DestructiveScope.kt" to
                """
                package com.example.devicemanagement.destructive
                enum class DestructiveScope { DEVICE_FACTORY_RESET }
                """.trimIndent(),
            "com/example/devicemanagement/destructive/DestructiveArtifactIdentity.kt" to
                """
                package com.example.devicemanagement.destructive
                class DestructiveArtifactIdentity
                """.trimIndent(),
            "com/example/devicemanagement/destructive/DestructiveOperatorChallenge.kt" to
                """
                package com.example.devicemanagement.destructive
                class DestructiveOperatorChallenge
                """.trimIndent(),
            "com/example/devicemanagement/destructive/DestructiveAttemptLease.kt" to
                """
                package com.example.devicemanagement.destructive
                class DestructiveAttemptLease
                """.trimIndent(),
            "com/example/devicemanagement/destructive/DestructiveHumanConfirmationResult.kt" to
                """
                package com.example.devicemanagement.destructive
                sealed class DestructiveHumanConfirmationResult
                """.trimIndent(),
            "com/example/devicemanagement/destructive/DestructiveTrustedPerAttemptConfirmationRecordSource.kt" to
                """
                package com.example.devicemanagement.destructive
                interface DestructiveTrustedPerAttemptConfirmationRecordSource
                """.trimIndent(),
            "com/example/devicemanagement/destructive/DestructiveUtcClock.kt" to
                """
                package com.example.devicemanagement.destructive
                interface DestructiveUtcClock
                """.trimIndent(),
            "com/example/devicemanagement/destructive/DestructiveTrustedApprovedBuildRevisionSource.kt" to
                """
                package com.example.devicemanagement.destructive
                interface DestructiveTrustedApprovedBuildRevisionSource
                """.trimIndent(),
            "com/example/devicemanagement/destructive/DestructiveLiveFactsSource.kt" to
                """
                package com.example.devicemanagement.destructive
                interface DestructiveLiveFactsSource
                """.trimIndent(),
            "com/example/devicemanagement/destructive/DestructiveTrustedArtifactExpectationSource.kt" to
                """
                package com.example.devicemanagement.destructive
                interface DestructiveTrustedArtifactExpectationSource
                """.trimIndent(),
            "com/example/devicemanagement/destructive/ProductionDestructiveHumanConfirmationSource.kt" to
                """
                package com.example.devicemanagement.destructive
                class ProductionDestructiveHumanConfirmationSource(
                    recordSource: DestructiveTrustedPerAttemptConfirmationRecordSource,
                    utcClock: DestructiveUtcClock,
                    approvedBuildRevision: DestructiveTrustedApprovedBuildRevisionSource,
                    liveFacts: DestructiveLiveFactsSource,
                    artifactExpectationSource: DestructiveTrustedArtifactExpectationSource,
                ) {
                    fun confirm(
                    ...
                    ): DestructiveHumanConfirmationResult? = null
                }
                """.trimIndent(),
        )
    }

    private fun productionInternalInitStubs(): Array<Pair<String, String>> {
        return arrayOf(
            *productionConfirmationStubs(),
            "com/example/devicemanagement/integration/MonotonicTimeSource.java" to
                """
            ...
            "com/example/devicemanagement/destructive/AuthorizedFactoryResetPort.java" to
                ...
                public interface AuthorizedFactoryResetPort {}
                """.trimIndent(),
            "com/example/devicemanagement/destructive/ProductionDestructiveTrustedArtifactExpectationSource.java" to
                """
                package com.example.devicemanagement.destructive;
                public final class ProductionDestructiveTrustedArtifactExpectationSource {
                    public ProductionDestructiveTrustedArtifactExpectationSource() {}
                }
                """.trimIndent(),
            "com/example/devicemanagement/destructive/ProductionDestructiveTrustedPerAttemptConfirmationRecordSource.java" to
                """
                package com.example.devicemanagement.destructive;
                public final class ProductionDestructiveTrustedPerAttemptConfirmationRecordSource {
                    public ProductionDestructiveTrustedPerAttemptConfirmationRecordSource() {}
                }
                """.trimIndent(),
            "com/example/devicemanagement/destructive/ProductionDestructiveUtcClock.java" to
                """
                package com.example.devicemanagement.destructive;
                public final class ProductionDestructiveUtcClock {
                    public ProductionDestructiveUtcClock() {}
                }
                """.trimIndent(),
            "com/example/devicemanagement/destructive/ProductionDestructiveApprovedBuildRevisionSource.java" to
                """
                package com.example.devicemanagement.destructive;
                public final class ProductionDestructiveApprovedBuildRevisionSource {
                    public ProductionDestructiveApprovedBuildRevisionSource() {}
                }
                """.trimIndent(),
            "com/example/devicemanagement/destructive/ProductionDestructiveRealChainOrchestrator.java" to
                """
                ...
                import com.example.devicemanagement.integration.MonotonicTimeSource;
                public final class ProductionDestructiveRealChainOrchestrator {
                    public ProductionDestructiveRealChainOrchestrator(
                        AndroidFutureDestructiveExecutor executor,
                        DestructiveLiveFactsSource liveFacts,
                        MonotonicTimeSource clock,
                        RuntimeDestructiveSafetyDurability durability,
                        DestructiveTrustedArtifactExpectationSource artifactExpectationSource,
                        ProductionDestructiveHumanConfirmationSource confirmationSource
                    ) {}
                }
                """.trimIndent(),
        )
    ...
    private fun authorizedRetainConstructionJava(): Pair<String, String> {
        return "com/example/devicemanagement/destructive/ProductionDestructiveRealChain.java" to
            """
            package com.example.devicemanagement.destructive;
            import com.example.devicemanagement.integration.MonotonicTimeSource;
            public final class ProductionDestructiveRealChain {
                public ProductionDestructiveRetainer retainForProduction(
                    ...
                    new ProductionDestructiveTrustedArtifactExpectationSource();
                    new ProductionDestructiveTrustedPerAttemptConfirmationRecordSource();
                    new ProductionDestructiveUtcClock();
                    new ProductionDestructiveApprovedBuildRevisionSource();
                    new ProductionDestructiveHumanConfirmationSource(null, null, null, null, null);
                    new ProductionDestructiveRealChainOrchestrator(
                        null, liveFacts, clock, durability, null, null
                    );
                    return null;
                }
            }
    ...
    private fun authorizedFactoryResetResultStub(): Pair<String, String> {
        return "com/example/devicemanagement/destructive/AuthorizedFactoryResetResult.java" to
            """
            package com.example.devicemanagement.destructive;
            public class AuthorizedFactoryResetResult {}
            """.trimIndent()
    ...
    private fun factoryResetWipeJava(body: String): Pair<String, String> {
        return "com/example/devicemanagement/management/AndroidDevicePolicyFactoryResetService.java" to
            """
            package com.example.devicemanagement.management;
            import android.app.admin.DevicePolicyManager;
            import com.example.devicemanagement.destructive.AuthorizedFactoryResetResult;
            public final class AndroidDevicePolicyFactoryResetService {
                private final DevicePolicyManager manager;
                AndroidDevicePolicyFactoryResetService(DevicePolicyManager manager) {
                    this.manager = manager;
                }
                public AuthorizedFactoryResetResult performAuthorizedFactoryReset() {
                    $body
    ...
    private fun factoryResetWipeDeviceBytecode(
        extraWipeDeviceZero: Boolean = false,
        emitFlags: MethodVisitor.() -> Unit,
        ...
        method.visitEnd()
        writer.visitEnd()
        return "$owner.class" to writer.toByteArray()
    }

    private fun factoryResetWipeDeviceHandleBytecode(): Pair<String, ByteArray> {
    ...
    private fun writeClassFiles(vararg classes: Pair<String, ByteArray>): File {
        val root = Files.createTempDirectory("policy-asm-fixture").toFile()
